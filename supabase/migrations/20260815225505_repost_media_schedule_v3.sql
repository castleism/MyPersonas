...
create or replace function public.create_repost(
  p_persona_id uuid, p_source_persona_id uuid default null, p_source_url text default '',
  p_source_platform text default '', p_attribution_text text default '',
  p_repost_type text default 'quote', p_notes text default ''
) returns uuid
language plpgsql security definer set search_path = '' as $$
declare v_owner uuid; v_persona_owner uuid; v_source_owner uuid; v_repost_id uuid;
begin
  v_owner := auth.uid();
  ...
  if p_repost_type not in ('quote','share','remix','response') then raise exception 'Invalid repost type'; end if;
  insert into public.persona_reposts (owner, persona_id, source_persona_id, source_url, source_platform, attribution_text, repost_type, status, notes)
  values (v_owner, p_persona_id, p_source_persona_id, p_source_url, p_source_platform, p_attribution_text, p_repost_type, 'draft', p_notes)
  returning id into v_repost_id;
  return v_repost_id;
end;
$$;

create or replace function public.link_repost_to_draft(p_repost_id uuid, p_draft_id uuid) returns void
language plpgsql security definer set search_path = '' as $$
declare v_owner uuid; v_draft_owner uuid;
begin
  v_owner := auth.uid();
  if v_owner is null then raise exception 'Authentication required'; end if;
  ...
  update public.persona_reposts set post_draft_id = p_draft_id, status = 'queued', updated_at = now() where id = p_repost_id;
end;
$$;

create or replace function public.get_content_calendar(p_days_ahead int default 7)
returns table (
...
language sql security definer stable set search_path = '' as $$
  select d.cal_date, t.persona_id, p.name, p.handle, 'scheduled_task' as item_type, t.name as task_name, t.content_kind, t.destination as platform, to_char(t.schedule_time, 'HH24:MI') as scheduled_time, t.last_status as status, null::uuid as draft_id, t.id as task_id
  from generate_series(current_date, current_date + (p_days_ahead - 1), '1 day'::interval) as d(cal_date)
  join public.ai_tasks t on t.active = true and t.owner = auth.uid() and ((t.cadence = 'daily') or (t.cadence = 'weekly' and extract(dow from d.cal_date) = coalesce(t.schedule_day, extract(dow from current_date))) or (t.cadence = 'manual' and t.next_run_at::date = d.cal_date))
  join public.personas p on p.id = t.persona_id
  union all
  select coalesce(pd.approved_at::date, pd.updated_at::date) as cal_date, pd.persona_id, p.name, p.handle, 'post_draft' as item_type, left(coalesce(pd.brief, ''), 60) as task_name, '' as content_kind, '' as platform, to_char(pd.approved_at, 'HH24:MI') as scheduled_time, pd.status, pd.id as draft_id, null::uuid as task_id
  from public.post_drafts pd
  join public.personas p on p.id = pd.persona_id
  where pd.owner = auth.uid() and pd.status in ('approved','posted') and coalesce(pd.approved_at, pd.updated_at) >= current_date and coalesce(pd.approved_at, pd.updated_at) < current_date + p_days_ahead
  order by cal_date, scheduled_time;
$$;

create or replace function public.add_media_asset(
  p_persona_id uuid, p_media_type text default 'image', p_storage_path text default '',
  p_public_url text default '', p_alt_text text default '', p_caption text default '',
  p_source text default 'generated', p_generation_prompt text default '',
  p_generation_backend uuid default null, p_tags text[] default '{}', p_metadata jsonb default '{}'::jsonb
) returns uuid
language plpgsql security definer set search_path = '' as $$
declare v_owner uuid; v_persona_owner uuid; v_asset_id uuid;
begin
  v_owner := auth.uid();
  if v_owner is null then raise exception 'Authentication required'; end if;
  select owner into v_persona_owner from public.personas where id = p_persona_id;
  if v_persona_owner is null then raise exception 'Persona not found'; end if;
  if v_persona_owner <> v_owner then raise exception 'Persona does not belong to you'; end if;
  if p_media_type not in ('image','video','audio','document') then raise exception 'Invalid media type'; end if;
  if p_source not in ('generated','uploaded','sourced','remixed') then raise exception 'Invalid source'; end if;
  insert into public.persona_media_assets (owner, persona_id, media_type, storage_path, public_url, alt_text, caption, source, generation_prompt, generation_backend, tags, metadata)
  values (v_owner, p_persona_id, p_media_type, p_storage_path, p_public_url, p_alt_text, p_caption, p_source, p_generation_prompt, p_generation_backend, p_tags, p_metadata)
  returning id into v_asset_id;
  return v_asset_id;
end;
$$;

create or replace function public.get_persona_media_library(p_persona_id uuid, p_media_type text default null)
returns table (id uuid, media_type text, storage_path text, public_url text, alt_text text, caption text, source text, tags text[], status text, created_at timestamptz)
language sql security definer stable set search_path = '' as $$
  select m.id, m.media_type, m.storage_path, m.public_url, m.alt_text, m.caption, m.source, m.tags, m.status, m.created_at
  from public.persona_media_assets m
  where m.owner = auth.uid() and m.persona_id = p_persona_id and (p_media_type is null or m.media_type = p_media_type) and m.status = 'active'
