...
async function renderPersonaModeProfile(handle, skipGate = false, epoch = ++renderEpoch) {
  ...
  const payload = result.data, p = payload?.persona;if (!p) { app.innerHTML = '<div class="empty">This page is not visible to the acting persona.</div>';return; }const ownedPersona=(myPersonas||[]).find(row=>row.id===p.id);if(ownedPersona){for(const field of["avatar_media_asset_id","banner_media_asset_id","bg_media_asset_id","feed_media_asset_id"])p[field]=ownedPersona[field]||p[field]||null}
