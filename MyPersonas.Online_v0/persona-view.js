...
function personaModeConfiguredPageHtml(payload, fanAvailable, isSelf, canAct) {
  const p = payload.persona, links = payload.links || [], posts = payload.posts || [], albums = payload.albums || [], relationCards = payload.relation_cards || [], revenue = payload.revenue || null, customFields = normalizePersonaCustomFields(payload.custom_field_boxes);
  ...
  const albumHtml = albums.map((album) => `<div class="card"><h3>${ico(album.kind === "affiliate" ? "cart" : "album")}${esc(album.title)} ${album.kind === "affiliate" ? '<span class="pill priv">sponsored</span>' : ""}</h3><div class="albgrid">${(album.items || []).map((item) => { const source = safeHttpUrl(item.thumb_url), thumb = safePublicMediaUrl(item.thumb_url), link = safeExternalNavigationUrl(item.link_url), mark = MyPersonasAiProvenance.publicMarkerHtml(thumb, "image"), inner = `${thumb || isSelf && item.media_asset_id ? `<img class="${isSelf ? "persona-asset " : ""}${MyPersonasAiProvenance.imagePresentationClass(thumb)}"${thumb ? ` src="${esc(thumb)}"` : ""} loading="lazy" draggable="false" alt="${esc(item.caption || album.title)}" ${personaAssetAttributes(source, item.caption || album.title, isSelf, "image", item.media_asset_id)}${personaViewerMediaAttributes(thumb,isSelf)}>${mark}` : ""}<span class="cap">${esc(item.caption) || "&nbsp;"}</span>${link ? '<span class="goicon">↗</span>' : ""}`;return link ? `<a class="albitem${mark ? " has-ai-provenance" : ""}" href="${esc(link)}" target="_blank" rel="noopener${album.kind === "affiliate" ? " sponsored" : ""}">${inner}</a>` : `<span class="albitem${mark ? " has-ai-provenance" : ""}">${inner}</span>`; }).join("") || '<p class="muted">empty album</p>'}</div></div>`).join("");
  return `${isSelf && canAct ? composerHtml(p) : ""}<div class="persona-layout-grid" style="--pc:${safeTheme(p.theme)}">
    ${M("live") && safeExternalMediaEmbedUrl(p.live_url) ? `<section class="${personaLayoutItemClass(pageLayout, "live")}" style="${personaLayoutItemStyle(pageLayout, "live")}"><div class="card livebox"><h3>${ico("live")}Live now</h3>${liveHtml(p.live_url)}</div></section>` : ""}
    ${M("music") && safeExternalMediaEmbedUrl(p.music_url) ? `<section class="${personaLayoutItemClass(pageLayout, "music")}" style="${personaLayoutItemStyle(pageLayout, "music")}"><div class="card music"><h3>${ico("music")}Profile song</h3>${musicHtml(p.music_url)}</div></section>` : ""}
    ...
    ${M("links") && links.length ? `<section class="${personaLayoutItemClass(pageLayout, "links")}" style="${personaLayoutItemStyle(pageLayout, "links")}"><div class="card"><h3>${ico("link")}Find me on</h3>${links.map((link) => { const platform = PLATS[link.platform] || PLATS.other, url = safeExternalNavigationUrl(link.url);return url ? `<a class="chip" href="${esc(url)}" target="_blank" rel="noopener noreferrer">${ico("node")}${esc(link.handle || platform.name)}</a>` : ""; }).join("")}</div></section>` : ""}
    ${M("top8") && top8.length ? `<section class="${personaLayoutItemClass(pageLayout, "top8")}" style="${personaLayoutItemStyle(pageLayout, "top8")}"><div class="card"><h3>${ico("crown")}Top ${top8.length}</h3><div class="top8grid">${top8.map((relative) => { const mark = MyPersonasAiProvenance.publicMarkerHtml(safePublicMediaUrl(relative.avatar_url), "image");return `<div class="t8" data-handle="${esc(relative.handle)}" onclick="go('p/'+this.dataset.handle)"><div class="av8 ai-asset-shell${mark ? " has-ai-provenance" : ""}" style="${safeBgStyle(relative.avatar_url)}">${mark}</div>${esc(relative.name)}</div>`; }).join("")}</div></div></section>` : ""}
    ${M("linked") && linked.length ? `<section class="${personaLayoutItemClass(pageLayout, "linked")}" style="${personaLayoutItemStyle(pageLayout, "linked")}"><div class="card"><h3>${ico("mask")}More of me</h3>${linked.map((relative) => `<div class="t8pick" style="cursor:pointer" data-handle="${esc(relative.handle)}" onclick="go('p/'+this.dataset.handle)">${personaRelationAvatarHtml(relative.avatar_url)}<div><b>${esc(relative.name)}</b><br><span class="muted">${esc(relative.tagline || "@" + relative.handle)}</span></div></div>`).join("")}</div></section>` : ""}
    ${M("family") && family.length ? `<section class="${personaLayoutItemClass(pageLayout, "family")}" style="${personaLayoutItemStyle(pageLayout, "family")}"><div class="card"><h3>${ico("user")}Family</h3>${family.map((relative) => `<div class="t8pick" style="cursor:pointer" data-handle="${esc(relative.handle)}" onclick="go('p/'+this.dataset.handle)">${personaRelationAvatarHtml(relative.avatar_url)}<div><b>${esc(relative.name)}</b><br><span class="muted">${esc(relative.relationship_label || "family")}</span></div></div>`).join("")}</div></section>` : ""}
...
async function renderPersonaModeProfile(handle, skipGate = false, epoch = ++renderEpoch) {
  ...
  const payload = result.data, p = payload?.persona;if (!p) { app.innerHTML = '<div class="empty">This page is not visible to the acting persona.</div>';return; }const customFieldResult=await sb.rpc("persona_custom_field_boxes",{p_persona_id:p.id,p_actor_persona_id:actor.id});if(epoch!==renderEpoch||!ownerAppPerspectiveSnapshotCurrent(snapshot))return;payload.custom_field_boxes=customFieldResult.error?[]:customFieldResult.data;const ownedPersona=(myPersonas||[]).find(row=>row.id===p.id);if(ownedPersona){for(const field of["avatar_media_asset_id","banner_media_asset_id","bg_media_asset_id","feed_media_asset_id"])p[field]=ownedPersona[field]||p[field]||null}
  ...
  if (epoch !== renderEpoch || !ownerAppPerspectiveSnapshotCurrent(snapshot)) return;
  const safeBackground = safePublicMediaUrl(p.bg_url), backgroundMark = MyPersonasAiProvenance.publicMarkerHtml(safeBackground, "image"), bannerMark = MyPersonasAiProvenance.publicMarkerHtml(safePublicMediaUrl(p.banner_url), "image"), avatarMark = MyPersonasAiProvenance.publicMarkerHtml(safePublicMediaUrl(p.avatar_url), "image");if (safeBackground) { document.body.classList.add("custom-bg");document.body.style.backgroundImage = `url(${JSON.stringify(safeBackground)})`; }
  ...
  app.innerHTML = `<div class="persona-peer-shell"><div class="persona-mode-notice ${canAct ? "" : "readonly"}"><b>Acting as ${esc(actor.name)}.</b> ${esc(personaViewActorStatus(actor, payload.actor || {}))}</div>
    ${backgroundMark ? `<div class="page-background-provenance">${backgroundMark}</div>` : ""}<div class="banner ai-asset-shell${bannerMark ? " has-ai-provenance" : ""}${isSelf && safeHttpUrl(p.banner_url) ? " persona-asset" : ""}" style="--pc:${safeTheme(p.theme)};${safeBgStyle(p.banner_url)}" ${personaAssetAttributes(p.banner_url, "Banner image", isSelf, "image", p.banner_media_asset_id)}${personaViewerMediaAttributes(p.banner_url,isSelf)}>${bannerMark}</div><div class="pp-head"><div class="pp-av ai-asset-shell${avatarMark ? " has-ai-provenance" : ""}${isSelf && safeHttpUrl(p.avatar_url) ? " persona-asset" : ""}" style="${safeBgStyle(p.avatar_url)}" ${personaAssetAttributes(p.avatar_url, "Profile image", isSelf, "image", p.avatar_media_asset_id)}${personaViewerMediaAttributes(p.avatar_url,isSelf)}>${avatarMark}</div><div style="flex:1;padding-bottom:6px"><div class="pp-name">${esc(p.name)} ${p.pet_project ? `<span class="pill" style="background:#eafff1;color:#0a7d33">★ ${esc(p.pet_project)}</span>` : ""} ${p.nsfw ? '<span class="pill nsfw">18+</span>' : ""} ${p.visibility !== "public" ? `<span class="pill priv">${esc(p.visibility)}</span>` : ""} ${safeExternalMediaEmbedUrl(p.live_url) ? '<span class="pill live">LIVE</span>' : ""}</div><div class="muted" style="background:rgba(255,255,255,.86);display:inline-block;padding:2px 8px;border-radius:8px">@${esc(p.handle)} · ${isSelf ? "your acting page" : payload.relationship?.friendship_status === "accepted" ? `friend of @${esc(actor.handle)}` : `viewed as @${esc(actor.handle)}`} — ${esc(p.tagline || "")}</div></div><div class="persona-peer-actions">${personaPeerRelationshipButtons(payload)}<button class="btn sec sm" onclick="sharePage('${esc(p.handle)}')">Share</button>${isSelf && (safeHttpUrl(p.bg_url)||p.bg_media_asset_id) ? `<button class="btn sec sm" ${personaAssetAttributes(p.bg_url,"Page background",true,"image",p.bg_media_asset_id)}>Preview background</button>` : ""}${!sameOwner ? `<button class="btn danger sm" title="Account-wide: affects every persona you own" onclick="blockPersona('${p.id}','block')">Block for all my personas</button><button class="btn sec sm" title="Account-wide: affects every persona you own" onclick="blockPersona('${p.id}','mute')">Mute for all my personas</button>` : ""}</div></div>
    ...
    ${personaModeConfiguredPageHtml(payload, fanAvailable, isSelf, canAct)}</div>`;hydrateOwnerMediaPreviews(app);const bgAssetId=isSelf?String(p.bg_media_asset_id||""):"",uid=session?.user?.id||"",generation=ownerMediaAuthGeneration;if(uid&&(bgAssetId||PUBLIC_MEDIA_V1.test(safePublicMediaUrl(p.bg_url))))renewOwnerMediaPreview(p.bg_url,bgAssetId).then(preview=>{if(pageState?.p?.id===p.id&&ownerMediaAuthStillCurrent(uid,generation)&&ownerMediaBlobUrls.has(preview.url)){document.body.classList.add("custom-bg");document.body.style.backgroundImage=`url(${JSON.stringify(preview.url)})`}}).catch(()=>{});
}
