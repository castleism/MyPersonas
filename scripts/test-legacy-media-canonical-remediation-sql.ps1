...
$taskContainer = "mypersonas-legacy-media-065-test"
$taskCreated = $false
$taskExisting = @(& docker ps -a --format "{{.Names}}")
if ($LASTEXITCODE -ne 0) {
  throw "Docker is required for the legacy-media canonical remediation SQL test."
}
...
try {
  ...
  $taskFiles = @(
    ...
    "MyPersonas.Online_v0/sql-updates/064-legacy-media-remediation.sql",
    "tests/sql/065-legacy-media-canonical-remediation-seed.sql",
    "MyPersonas.Online_v0/sql-updates/065-legacy-media-canonical-remediation.sql",
    "MyPersonas.Online_v0/sql-updates/065-legacy-media-canonical-remediation.sql",
    "tests/sql/065-legacy-media-canonical-remediation-runtime.sql"
  )
  ...
  Write-Output "Migrations 059-065, repeated forward application, and legacy declaration/import/CAS/clear/readiness/isolation assertions passed."
}
finally {
