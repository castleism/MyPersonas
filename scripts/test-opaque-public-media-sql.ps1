...
$taskContainer = "mypersonas-opaque-media-062-test"
$taskCreated = $false

$taskExisting = @(& docker ps -a --format "{{.Names}}")
if ($LASTEXITCODE -ne 0) { throw "Docker is required for the opaque media SQL runtime test." }
...
try {
  ...
  Write-Output "Frozen provenance migrations upgraded through 062, 062 reapplied, and opaque lifecycle/role assertions passed."
}
finally {
