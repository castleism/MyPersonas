...
test("billing deployment keeps test Checkout off the configured production project",()=>{
  assert.match(deployWorkflow,/billing_staging_project_ref:/);
  ...
  assert.match(deployWorkflow,/name: \$\{\{ inputs\.release_scope == 'billing-test-boundary' && 'billing-staging' \|\| 'production' \}\}/);
  assert.match(deployWorkflow,/BILLING_STAGING_SUPABASE_ACCESS_TOKEN/);
  ...
  assert.match(deployWorkflow,/APPROVED_BILLING_STAGING_PROJECT_REF: \$\{\{ secrets\.BILLING_STAGING_PROJECT_REF \}\}/);
